...
use crate::validation::{validate_tool_name, validate_resource_uri, validate_prompt_name};
use axum::{
    extract::State,
    response::Json,
    routing::{get, post},
...
impl McpServer {
    ...
    pub fn register_tool(&mut self, name: String, tool: Arc<dyn Tool>) -> Result<(), McpError> {
    ...
    pub fn register_resource(&mut self, name: String, resource: Arc<dyn Resource>) -> Result<(), McpError> {
    ...
    pub fn register_prompt(&mut self, name: String, prompt: Arc<dyn Prompt>) -> Result<(), McpError> {
...
async fn call_tool(
    ...
    match result {
        Ok(result_value) => {
            let text = serde_json::to_string(&result_value)
                .map_err(|e| {
...
async fn read_resource(
    ...
    let read_result = resource.read().await;
    let mime_type = resource.mime_type().to_string();
    match read_result {
...
async fn get_prompt(
    ...
    let render_result = prompt.render(&arguments).await;
    match render_result {
        Ok(content) => {
            Ok(Json(serde_json::json!({
                "messages": [{
